...
const count = (target, input) => {
  // Solution code here...
  return input.reduce((accum, arr) => {

    let stuff = arr.reduce((accum2, value) =>{
...
const totalSum = (input) => {
  // Solution code here...
  return input.reduce((accum, arr) => accum += arr.reduce((accum2, value) => accum2 += value), 0);
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let filtered = data.filter(char => char.gender === 'male' | char.gender === 'female')
  let names = filtered.map(char => {
    return char.name;
  })
...
let findShortest = (data) => {
  // Solution code here...
  let shorty = data.reduce((accum, val) => {
    if(accum.name.length >= val.name.length){
      accum = val
    }
    return accum;
  }, data[0])
